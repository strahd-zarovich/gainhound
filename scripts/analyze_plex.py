...
"""
===============================================================================
Script: analyze_plex.py
Purpose: Clears Plex analysis data for all music libraries using Plex API
...
LOG_FILE = os.path.join(LOG_DIR, "analyze_plex.log")
...
def clear_analysis():
    ...
    try:
        ...
        log("Clearing track analysis data...")
        for section in music_sections:
            section_id = section.get("key")
            section_title = section.get("title", "Unknown")
            if section_id:
                log(f"Clearing analysis in '{section_title}' (section {section_id})")
                clear_url = f"{PLEX_URL}/library/sections/{section_id}/unanalyze?X-Plex-Token={PLEX_TOKEN}"
